/**
 * This program and the accompanying materials
 * are made available under the terms of the License
 * which accompanies this distribution in the file LICENSE.txt
 */
package com.archimatetool.script.dom.model;

/**
 * Interface for model constants
 * 
 * @author Phillip Beauvoir
 */
public interface IModelConstants {

    String ID = "id"; //$NON-NLS-1$
    String NAME = "name"; //$NON-NLS-1$
    String DOCUMENTATION = "documentation"; //$NON-NLS-1$
    String PURPOSE = "purpose"; //$NON-NLS-1$
    String TYPE = "type"; //$NON-NLS-1$
    
    String ARCHIMATE_CONCEPT = "archimateConcept"; //$NON-NLS-1$
    String DIAGRAM_MODEL = "diagramModel"; //$NON-NLS-1$
    
    String SOURCE = "source"; //$NON-NLS-1$
    String TARGET = "target"; //$NON-NLS-1$
    
    String CONCEPTS = "concepts"; //$NON-NLS-1$
    String ELEMENTS = "elements"; //$NON-NLS-1$
    String RELATIONSHIPS = "relationships"; //$NON-NLS-1$
    
    String CHILDREN = "children";  //$NON-NLS-1$
 
    String SOURCE_RELATIONSHIPS = "sourceRelationships"; //$NON-NLS-1$
    String TARGET_RELATIONSHIPS =  "targetRelationships"; //$NON-NLS-1$
    String DIAGRAM_COMPONENT_INSTANCES =  "diagramComponentInstances"; //$NON-NLS-1$

    String SOURCE_CONNECTIONS = "sourceConnections"; //$NON-NLS-1$
    String TARGET_CONNECTIONS =  "targetConnections"; //$NON-NLS-1$

    String BOUNDS = "bounds"; //$NON-NLS-1$
    String FILL_COLOR = "fillColor";  //$NON-NLS-1$
    String FONT_COLOR = "fontColor";  //$NON-NLS-1$
    String FONT = "font";  //$NON-NLS-1$
    String LINE_COLOR = "lineColor";  //$NON-NLS-1$
    String LINE_WIDTH = "lineWidth";  //$NON-NLS-1$
}
